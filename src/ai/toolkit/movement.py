...
def next_step_to_target(current_tile: Tile, target_tile: Tile, domain: List[Tile]) -> Optional[Tuple[Tile, int]]:
    """
    ...
    if not target_tile:
        ...
        return None
    path = essentials.dijkstra_pq(current_tile, target_tile, domain)
    if len(path) <= 1:  # no path found
        return None
    return path[1], len(path) - 1          # return next step and distance of the path


def evasive_movement(current_tile: Tile, target_tile: Tile, domain: List[Tile]) -> Optional[Tuple[Tile, int]]:
    """
...
def protective_movement(current_tile: Tile, target_tile: Tile, protected_tile: Tile,
                          domain: List[Tile]) -> Optional[Tuple[Tile, int]]:
    """
