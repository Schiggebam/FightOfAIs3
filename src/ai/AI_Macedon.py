...
from typing import Set, List, Callable, Tuple, Optional, Union
...
class AI_Mazedonian(AI):
    ...
    Option = Union[BuildOption, RecruitmentOption, ScoutingOption, WaitOption]
    ...
    def __init__(self, name: str, own_id: int, other_players: [int]):
        ...
        bo_early_game: List[Tuple[BuildingType, int]] = [(BuildingType.FARM, 3), (BuildingType.HUT, 2), (BuildingType.BARRACKS, 2)]
        bo_mid_game: List[Tuple[BuildingType, int]] = [(BuildingType.FARM, 6), (BuildingType.HUT, 5), (BuildingType.BARRACKS, 4)]
        bo_late_game: List[Tuple[BuildingType, int]] = [(BuildingType.FARM, 7), (BuildingType.HUT, 7), (BuildingType.BARRACKS, 10)]
        self.ac_passive: AI_Mazedonian.ArmyConstellation = AI_Mazedonian.ArmyConstellation("passive", (0.5, 0.5), 15)
        self.ac_aggresive: AI_Mazedonian.ArmyConstellation = AI_Mazedonian.ArmyConstellation("aggressive", (0.8, 0.2), 15)
        self.ac_defencive: AI_Mazedonian.ArmyConstellation = AI_Mazedonian.ArmyConstellation("defencive", (0.3, 0.7), 15)
        ...
        self.weights.append(AI_Mazedonian.Weight(w1, 10))
        self.weights.append(AI_Mazedonian.Weight(w2, 10))
    ...
    def do_move(self, ai_stat: AI_GameStatus, move: AI_Move):
        ...
        # TODO upgrade
        recruitment_options: List[AI_Mazedonian.RecruitmentOption] = self.evaluate_move_recruitment(ai_stat)
        ...
        for opt in options:
            for w in self.weights:
                if w.condition(opt, ai_stat):
                    opt.weighted_score = opt.weighted_score + w.weight * opt.score.value
        options.sort(key=lambda x: x.weighted_score, reverse=False)
    ...
    def evaluate_move_building(self, ai_stat: AI_GameStatus) -> List[BuildOption]:
        ...
        if current_bo != None:
            ...
            return [AI_Mazedonian.BuildOption(BuildingType.FARM, site_farm, normalize(value_farm)),
    ...
    def evaluate_move_recruitment(self, ai_stat: AI_GameStatus) -> List[RecruitmentOption]:
        options = []
    ...
    def evaluate_move_scouting(self, ai_stat: AI_GameStatus) -> List[ScoutingOption]:
        ...
        value = 0
        for so in options:
            dist1 = AI_Toolkit.getListDistanceOne(so, ai_stat.tiles_discovered)
        ...
        options.sort(key=lambda x: x.score, reverse=False)
        return options
    ...
    def __get_attack_target(self, ai_stat: AI_GameStatus):
        ...
        if len(self.priolist_targets) > 0:
            self.priolist_targets.sort(key=lambda x: x[0], reverse=False)
